-- spec docs/dimensions/customers_dimension.md section 9: "SCD2 interval coverage"
-- (Critical) — per customer_id, validity intervals must run contiguously from
...
gaps as (
    ...
    select customer_id, 'gap between versions' as violation
...
bounds as (
    ...
    select customer_id, 'bounds not covered' as violation
    from ordered_versions
